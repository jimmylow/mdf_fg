    ...
    if($var_loginid == "") { 
	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['procd']) && is_array($_POST['procd'])) 
     	{
           foreach($_POST['procd'] as $key) {
             ...
             //$var_cust = $defarr[2];
		         ...
		         $vartoday = date("Y-m-d H:i:s");
			       $sql  = "Update cinvttrf Set stat = 'C', upd_by = '$var_loginid', upd_on = '$vartoday' ";
		   ...
		   $backloc = "../invt/m_trf_mas.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
...
<body>
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 900px;" class="style2">
	 <legend class="title">COUNTER STOCK TRANSFER LISTING</legend>
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                $locatr = "ctransfer_mas.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
         ...
         <tr>
          ...
          <th class="tabheader" style="width: 139px">Transfer No.</th>
           <!-- <th class="tabheader" style="width: 139px">Ref. No.</th> -->
          <th class="tabheader" style="width: 106px">Transfer Date</th>
          <th class="tabheader" style="width: 106px">MM/YYYY</th>
		 ...
		 <tbody>
		 <?php 
		    $sql = "SELECT trf_id, trfdate, mthyr, stat, posted ";
		    $sql .= " FROM cinvttrf ";
    		$sql .= " ORDER BY trf_id";   
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$trf_id = htmlentities($rowq['trf_id']);
				//$label_number = htmlentities($rowq['label_number']);

				$trfdate = date('d-m-Y', strtotime($rowq['trfdate']));
				$urlpop = 'upd_cinvttrf.php';
				$urlvie = 'vm_cinvttrf.php';
              ...
              //echo '<td>'.$rowq['posted'].'</td>';
            	...
            	if ($var_accvie == 0){
            		...
            		echo '<td align="center"><a href="'.$urlvie.'?trf_id='.$trf_id.'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            	}
	            if ($var_accupd == 0){
	            	...
	            	if ($rowq['posted'] == "Y" || $rowq['stat'] == "C"){
		              ...
		              echo '<td align="center"><a href="'.$urlpop.'?trf_id='.$trf_id.'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
	            	}
	            }
