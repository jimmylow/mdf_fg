        ...
        $vartoday = date("Y-m-d H:i:s"); 
            ...
            $mattype     = $_POST['procotype'][$row];
						...
						$matendbal   = $_POST['procobalqty'][$row];
							...
							if ($matrtnqty == "" or $matrtnqty == " " or $matrtnqty ==NULL) { $matrtnqty =0; }
							if ($matdoqty= "" or $matdoqty== " " or $matdoqty==NULL) { $matdoqty=0; }
							if ($matsoldqty= "" or $matsoldqty== " " or $matsoldqty==NULL) { $matsoldqty =0; }
							if ($matshortqty= "" or $matshortqty== " " or $matshortqty==NULL) { $matshortqty=0; }
							if ($matoverqty= "" or $matoverqty== " " or $matoverqty==NULL) { $matoverqty=0; }
							if ($matadjqty= "" or $matadjqty== " " or $matadjqty==NULL) { $matadjqty=0; }
							if ($matadjqty= "" or $matadjqty== " " or $matadjqty==NULL) { $matadjqty=0; }

					//echo 'MAT - '. $matcode . "</br>";
					//echo 'qty - '. $matsoldqty . "</br>";
...

						{
							...
							$sql = "INSERT INTO csalesdet values 
						    		('$var_invno', '$matcode', '$matupri', '$mattype', '$matdoqty','$matsoldqty', '$matrtnqty',
             ...
             <?php
              ...
              if($domthyr <> "") {
                ...
                if($domth == 12) {
             	...
             	$sql = "SELECT sprocd, sprounipri, sptype FROM csalesdet";
...
//              echo $sql; break;
        ...
        <input name="procotype[]" class="tInput" id="procotype<?php echo $i; ?>" style="width: 48px;" value ="<?php echo $rowq['sptype']; ?>"></td>                
                <td>
				<input name="procodoqty[]" id="procodoqty<?php echo $i; ?>" style="border-style: none; width: 48px; text-align : right" value ="<?php echo $doqty; ?>" readonly>
        ...
        <input name="procotype[]" class="tInput" id="procotype<?php echo $i; ?>" style=" width: 48px;" value ="<?php echo $rowq['sptype']; ?>"></td>
                <td>
				<input name="procodoqty[]" id="procodoqty<?php echo $i; ?>" style="border-style: none; width: 48px; text-align : right" value ="<?php echo $doqty; ?>" readonly></td>
				<input id="begbal<?php echo $i; ?>" type="hidden" value ="<?php echo $begbal; ?>" readonly></td>
                <td>
        <input name="procosoldqty[]" class="tInput" id="procosoldqty<?php echo $i; ?>" style="width: 48px; text-align : right"  value ="<?php echo $var_soldqty; ?>" onBlur="getamt(<?php echo $i; ?>)"></td>                
                <td>
        <input name="procosamt[]" class="tInput" id="procosamt<?php echo $i; ?>" style="border-style: none; width: 48px; text-align : right" readonly="readonly" value ="<?php echo $var_salesamt; ?>"></td>                
