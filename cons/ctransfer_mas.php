    ...
    $vmmthyr  = $_POST['samthyr'];
		...
		//if ($refno <> "") {
			...
			$sysno = '';
     		...
     		$sqlchk.= " where `descrip` = 'CINVTTRF' and counter = 'HQ';";
     		...
     		{
     			$sysno = '0';
     					$sysno_sql = "INSERT INTO ctrl_sysno values ('CINVTTRF', 'HQ', 1)";
     		...
     		$trf_sysno = "CTRF".$trf_sysno;


         	$vartoday = date("Y-m-d H:i:s");
				$sql = "INSERT INTO cinvttrf values 
						('$trf_sysno', '$trfdte', '$vmfrmcd', '$vmtocd', '$vmmthyr', '$remark', 
            ...
            //$todesc     = addslashes($todesc);
						...
						if ($frmcode <> "" && $trfqty > 0)
						{
							...
							$sql = "INSERT INTO cinvttrfdet values 
						    		('$trf_sysno', '$seqno', '$frmcode', '$frmdesc', '$trfqty')";
               ...
               */
				...
				$updsysno_sql = "UPDATE ctrl_sysno SET noctrl = '$newsysno' where `descrip` = 'CINVTTRF' and counter = 'HQ'";
...
function validateForm()
{
  ...
  var x=document.forms["InpJobFMas"]["frmctr"].value;
	if (x==null || x=="s")
	{
	alert("From Counter Cannot Be Blank");
	document.InpJobFMas.frmctr.focus;
	return false;
	}
  
  var x=document.forms["InpJobFMas"]["toctr"].value;
	if (x==null || x=="s")
	{
	alert("To Counter Cannot Be Blank");
	document.InpJobFMas.toctr.focus;
	return false;
	} 
  
  var x=document.forms["InpJobFMas"]["samthyr"].value;           
	if (x==null || x=="")
	{
	alert("MM/YYYY Must Not Be Blank");
	document.InpJobFMas.samthyr.focus;
	return false;
	} 
    ...
    for (var j = 1; j < rowCount; j++){
       ...
       if (rowItem != ""){
       	var strURL="getconhand.php?i="+rowItem;
	   	var req = getXMLHTTP();
        if (req)
	  	{
			req.onreadystatechange = function()
...
<body onload= "setup(); setup2();">
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 900px;" class="style2">
	 <legend class="title">STOCK TRANSFER BETWEEN COUNTER</legend>
	  <br>	 
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
        ...
        echo '>'.$row['counter']." | ".$row['name'].'</option>';
	         ...
	         ?>				   
	   	   ...
	   	   <tr>
	   	    <td></td>
	  	    <td style="width: 126px">MM/YYYY</td>
		  ...
		  <table id="itemsTable" class="general-table" style="width: 857px">
            ...
            <tbody>
             ...
             <tr class="item-row">
                ...
                <td>
				<input name="procofrm[]" value="" tProItem1=1 id="procofrm1" tabindex="0" class="autosearch" style="width: 150px" onchange ="upperCase(this.id)"></td>
                <td>
				<input name="procofdesc[]" value="" class="tInput" id="procofdesc1" readonly="readonly" style="border-style: none; border-color: inherit; border-width: 0; width: 300px;"></td>
          <!--     <td>
