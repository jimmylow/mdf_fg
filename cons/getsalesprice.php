...
$d=$_GET['d'];
 ...
 if ($s <> "s") {
	...
	include("../Setting/Connection.php");
     ...
     $sql= " SELECT distinct selltype ";
     $sql .= " FROM product ";
     $sql .= " where groupcode = '".$i."'";
     ...
     $sql= " SELECT uprice ";
     $sql .= " FROM prodprice ";
     $sql .= " where productcode in (select productcode from product where groupcode = '$i')";
     ...
     if($d <> "") {
        ...
        $sql = "SELECT sum(qtyperpack)  as totalqty from consopen x, consopendet y";
	    $sql .= " WHERE x.opening_id = y.opening_id ";
	    $sql .= " AND y.item_code = '$i'";
	    $sql .= " AND custcode = '$s'";
	    ...
	    if($var_opening=="")
	    {
     	...
     	echo $var_type."~".$var_price."~".$doqty."~".$begbal."~".$var_opening; 
       //echo $sql;                 
		mysql_close($db_link);
  	} else {
    	echo "Xk0.00k0k0";
