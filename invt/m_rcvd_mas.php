           ...
           foreach($_POST['rcvdno'] as $key) {
             ...
             $var_cust = $defarr[2];
		         ...
		         $vartoday = date("Y-m-d H:i:s");
			       $sql  = "Update invtrcvd Set stat = 'C', modified_by = '$var_loginid', modified_on = '$vartoday' ";
   ...
   if ($_POST['Submit'] == "Active") {
     	if(!empty($_POST['rcvdno']) && is_array($_POST['rcvdno'])) 
     	{
           foreach($_POST['rcvdno'] as $key) {
		     ...
		     $vartoday = date("Y-m-d H:i:s");
			 $sql  = "Update invtrcvd Set stat = 'A', modified_by = '$var_loginid', modified_on = '$vartoday' ";
             $sql .=	" Where rcvdno ='".$var_sale."' And custcd='".$var_cust."'";
...
<body>
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 900px;" class="style2">
	 <legend class="title">GOODS RECEIVED (OTH SUPPLIER) LISTING</legend>
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                $locatr = "receive_mas.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
		 ...
		 <?php 
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$urlpop = 'upd_invtrcvd.php';
				$urlvie = 'vm_invtrcvd.php';
				//$urlvie = 'ship_mas.php';
