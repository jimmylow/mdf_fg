<?php
    ...
    $vmremark = $_POST['saremark'];
		...
		if ($vmcustcd <> "") {
            ...
            /*----------------------------- Cash Bill details ------------------------------------ */
              $chk_invno_query = mysql_query("select count(*) from `ctrl_sysno` where `descrip` = 'INVTRTN' and counter = 'HQ'; ", $db_link);

              $chk_invno_res = mysql_fetch_array($chk_invno_query) or die("cant Get Purchase Return Info".mysql_error());
              
              if ($chk_invno_res[0] > 0 ) {
                  $get_invno_query = mysql_query("select noctrl from `ctrl_sysno` where `descrip` = 'INVTRTN' and counter = 'HQ' ", $db_link);
 		  ...
 		  mysql_query("update `ctrl_sysno` set `noctrl` = `noctrl` + 1
                           where `descrip` = 'INVTRTN' and counter = 'HQ'", $db_link) 
                           or die("Cant Update Purchase Return Auto No ".mysql_error());              
               
                }  else { 

		   mysql_query("insert into `ctrl_sysno` 
                          (`descrip`, `counter`, `noctrl`)
                   values ('INVTRTN', 'HQ', 1);",$db_link) or die("Cant Insert Into Purchase Return Auto No ".mysql_error());
        ...
        $vartoday = date("Y-m-d H:i:s"); 
				...
				$sql = "INSERT INTO invtreturn values 
						('$var_invno', '$vmrtndte','$vmcustcd', '$vmrefno','$var_loginid','$vartoday', 
			...
			$backloc = "../invt/return_mas.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}  
 ...
 <!-- <?php include("../sidebarm.php"); ?> -->
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 973px;" class="style2">
	 <legend class="title">PURCHASE RETURN ENTRY</legend>
	  <br>	 
