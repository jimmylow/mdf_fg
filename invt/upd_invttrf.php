<?php
    ...
    $vmremark = htmlentities($_POST['saremark']);
        ...
        $vartoday = date("Y-m-d H:i:s"); 
				...
				$sql = "Update invttrf Set trfdate ='$vmtrfdte', ";
				$sql .= "                  remark = '$vmremark', ";
            ...
            $todesc    = addslashes($todesc);
						...
						if ($frmcode <> "" && $tocode <> "" && $trfqty)
						{
							
							$sql = "INSERT INTO invttrfdet values 
						    		('$vmordno', '$seqno', '$frmcode', '$frmdesc', '$tocode','$todesc','$trfqty')";
			...
			$backloc = "../invt/upd_invttrf.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}  
...
function validateForm()
{
  ...
  // ---- 07/12/2015 - temporary not checking this, allow then do all the posting 1st
...
function checkdate(pid) {
   var entereddate = document.forms["InpPO"]["trfdte"].value; 
 
   var date =  entereddate.substring(0,2);
   var mth =  entereddate.substring(3,5);
   var year =  entereddate.substring(6,10);
   
   var myDate = new Date(year, mth - 1, date);
   var today = new Date();
   
   //alert (myDate + " " + today);
   
   if(myDate > today) {
       alert("Date is greater than today's date");
       return false;
   }
}
...
<!--  <?php include("../sidebarm.php"); ?> -->
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 857px;" class="style2">
	 <legend class="title">UPDATE STOCK TRANSFER</legend>
	  <br>	 
	  ...
	  <form name="InpPO" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 993px; ">
		  ...
		  <tr>
	  	   ...
	  	   <td style="width: 13px">:</td>
	  	   <td style="width: 201px">
		   <input class="inputtxt" name="trfdte" id ="trfdte" type="text" style="width: 128px;" value="<?php  echo $trfdte; ?>" onblur="return checkdate(this.id);">
