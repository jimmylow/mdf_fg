<?php
    ...
    if ($_POST['Submit'] == "Save") {
     ...
     if ($catcd <> "") {
      ...
      if ($res_id->cnt > 0 ) {
	     $backloc = "../main_mas/category_mas.php?stat=3&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>"; 
      }else {
      
         $sql = "INSERT INTO category_master values 
                ('$catcd', '$catdesc', '$var_loginid', CURDATE(), '$var_loginid', CURDATE())";

     	 mysql_query($sql); 
     	 $backloc = "../main_mas/category_mas.php?stat=1&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";      
      } 
     }else{
       $backloc = "../main_mas/category_mas.php?stat=4&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";
     }
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['catcd']) && is_array($_POST['catcd'])) 
     {
           ...
           foreach($_POST['catcd'] as $value ) {
		    $sql = "DELETE FROM category_master WHERE category_code ='".$value."'"; 
		 	mysql_query($sql); 
		   }
		  $backloc = "../main_mas/category_mas.php?stat=1&menucd=".$var_menucode;
          echo "<script>";
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
        ...
        // Redirect browser
        $fname = "category_rpt.rptdesign&__title=myReport"; 
        ...
        $backloc = "../main_mas/category_mas.php?menucd=".$var_menucode;
        echo "<script>";
        echo 'location.replace("'.$backloc.'")';
        echo "</script>";
     }
