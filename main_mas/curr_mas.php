<?php
    ...
    if ($_POST['Submit'] == "Save") {
     ...
     $currdesc  = $_POST['currdesc'];
     
     if ($currcode <> "") {

      $var_sql = " SELECT count(*) as cnt from currency_master ";
      $var_sql .= " WHERE currcode = '$currcode'";
      ...
      if ($res_id->cnt > 0 ) {
         ...
         $sql = "INSERT INTO currency_master values 
                ('$currcode', '$currdesc', '$var_loginid', '$vartoday', '$var_loginid', '$vartoday')";
     	 mysql_query($sql); 
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['currcode']) && is_array($_POST['currcode'])) 
     {
        
           foreach($_POST['currcode'] as $value ) {
		    $sql = "DELETE FROM currency_master WHERE currcode ='".$value."'"; 
		 	mysql_query($sql); 
		   }
		   $backloc = "../main_mas/curr_mas.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
...
<body onload="document.InpTermMas.currcode.focus()">
	  ...
	  <form name="InpTermMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>" style="height: 134px; width: 916px;">
		<table>
	  	  <tr>
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="currcode" id ="currid" type="text" maxlength="30" onchange ="upperCase(this.id)"  onBlur="AjaxFunction(this.value);">
			</td>
	  	  </tr>
	  	  <tr>
	  	    <td></td> 
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="currdesc" id ="currdescid" type="text" maxlength="60" onchange ="upperCase(this.id)" style="width: 486px">
			</td>
	  	  </tr>  
	  	  <tr>
	  	   <td>
	    ...
	    <br/><br/>
	  	<?php
				$sql = "SELECT currcode, currdesc, modified_by, modified_on ";
        ...
        <form name="LstTermMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>">
         ...
         <thead>
         	...
         	<tr>
          		...
          		<th class="tabheader" style="width: 138px">Currency Code</th>
          		<th class="tabheader" style="width: 303px">Currency Description</th>
		 ...
		 <?php 
            ...
            echo '<td align="center"><a href="'.$urlpop.'?c='.$row['currcode'].'&d='.$row['currdesc'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }
            if ($var_accdel == 0){
              echo '<td align="center"><input type="checkbox" DISABLED  name="currcode[]" value="'.$rowq['currcode'].'" />'.'</td>';
            }else{
              echo '<td align="center"><input type="checkbox" name="currcode[]" value="'.$row['currcode'].'" />'.'</td>';
            }
