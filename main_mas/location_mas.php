<?php
    ...
    if ($_POST['Submit'] == "Save") {
     ...
     if ($loccd <> "") {
      ...
      $var_sql .= " WHERE location_code = '$loccd'";

      $query_id = mysql_query($var_sql) or die($var_stat = mysql_error());
      $res_id = mysql_fetch_object($query_id);

      if ($res_id->cnt > 0 ) {
	     $backloc = "../main_mas/location_mas.php?stat=3&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>"; 
      }else {
      
         $sql = "INSERT INTO location_master values 
                ('$loccd', '$locdesc', '$var_loginid', CURDATE(), '$var_loginid', CURDATE())";

     	 mysql_query($sql); 
     	 $backloc = "../main_mas/location_mas.php?stat=1&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";      
      } 
     }else{
       $backloc = "../main_mas/location_mas.php?stat=4&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";
     }
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['loccd']) && is_array($_POST['loccd'])) 
     {
        
           foreach($_POST['loccd'] as $value ) {
		    $sql = "DELETE FROM location_master WHERE location_code ='".$value."'"; 
		 	mysql_query($sql); 
		   }
		  $backloc = "../main_mas/location_mas.php?stat=1&menucd=".$var_menucode;
          echo "<script>";
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
        ...
        // Redirect browser
        $fname = "location_rpt.rptdesign&__title=myReport"; 
...
<body onload="document.InpCurrMas.loccd.focus();">
  <?php include("../topbarm.php"); ?> 
  <div class="contentc">

	<fieldset name="Group1" style=" width: 933px;" class="style2">
	 <legend class="title">LOCATION MASTER</legend>
	  <br>
	  ...
	  <form name="InpCurrMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" style="height: 134px; width: 907px;">
		<table>
	  	  <tr>
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="loccd" id ="locid" type="text" maxlength="10" onchange ="upperCase(this.id)" size = "12" onBlur="AjaxFunction(this.value);">
			</td>
	  	  </tr>
	  	  <tr>
	  	    <td>
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="locde" id ="locdeid" type="text" maxlength="50" onchange ="upperCase(this.id)" style="width: 354px">
			</td>
	  	  </tr>  
	  	  <tr>
	  	   <td>
	  	...
	  	<?php
				$sql = "SELECT location_code, location_desc, modified_by, modified_on ";
        ...
        <form name="LstColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>">
		 ...
		 <tr>
           <td style="width: 918px; height: 38px;" align="right">
             <?php
		       ...
		       $msgdel = "Are You Sure Delete Selected Location ID?";
		 ...
		 <table cellpadding="0" cellspacing="0" id="example" class="display" width="100%">
         	...
         	<tr>
          		...
          		<th class="tabheader" style="width: 138px">Location ID</th>
		 ...
		 <?php 
			...
			$urlpop = 'upd_location_mas.php';
			echo '<tr>';
            echo '<td>'.$numi.'</td>';
            echo '<td>'.$row['location_code'].'</td>';
            echo '<td>'.$row['location_desc'].'</td>';
            ...
            echo '<td align="center"><a href="'.$urlpop.'?loccd='.$row['location_code'].'&locde='.$row['location_desc'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }
            if ($var_accdel == 0){
              echo '<td align="center"><input type="checkbox" DISABLED  name="loccd[]" value="'.$rowq['location_code'].'" />'.'</td>';
            }else{
              echo '<td align="center"><input type="checkbox" name="loccd[]" value="'.$row['location_code'].'" />'.'</td>';
            }
            echo '</tr>';
            $numi = $numi + 1;
			}
