<?php
    ...
    if ($_POST['Submit'] == "Deactive") {
     if(!empty($_POST['ctrcd']) && is_array($_POST['ctrcd'])) 
     {
           $custmoby= $var_loginid;
           $custmoon= date("Y-m-d H:i:s");
           foreach($_POST['ctrcd'] as $value ) {
		    $sql = "Update counter set Status ='I',";
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
        ...
        $fname = "counter_rpt.rptdesign&__title=myReport"; 
	...
	<fieldset name="Group1" style=" width: 1200px;" class="style2">
	 <legend class="title">COUNTER MASTER</legend>
	  <br>
		 ...
		 <tr>
           ...
           <td style="width: 1209px; height: 38px;" align="left">
			   <?php
			   $locatr = "ctr_mas.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
    	  	   ...
    	  	   $msgdel = "Are You Sure Active Selected Counter Code?";
    	  	   include("../Setting/btnactive.php"); 
			   $msgdel = "Are You Sure Deactive Selected Counter Code?";
         ...
         <thead>
         	<tr>
         	 ...
         	 <th style="width: 68px">CounterNo</th>
         	...
         	<tr>
         	 ...
         	 <th class="tabheader" style="width: 68px; height: 35px;">Counter<br>Code</th>
		 ...
		 <tbody>
		 <?php 
		 	$sql = "SELECT counter, Name, Tel1, Fax1, Contact, fd_status, modified_by, modified_on ";
			$sql .= " FROM counter";
			...
			$urlpop = 'upd_ctr_mas.php';
			$urlvm = 'vm_ctr_mas.php';
      ...
      //else { $showdte = ""; }
            ...
            //echo '<td>'.$showdte.'</td>';  
            echo '<td>'.$row['fd_status'].'</td>';
			
			if ($var_accvie == 0){
            echo '<td align="center"><a href="#">[VIEW]</a>';'</td>';
            }else{
            echo '<td align="center"><a href="'.$urlvm.'?ctrcd='.$row['counter'].'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            }
			
			if ($var_accupd == 0){
            echo '<td align="center"><a href="#">[EDIT]</a>';'</td>';
            }else{
            echo '<td align="center"><a href="'.$urlpop.'?ctrcd='.$row['counter'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }

            echo '<td align="center"><input type="checkbox" name="ctrcd[]" value="'.$row['counter'].'" />'.'</td>';
