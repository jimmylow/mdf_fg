<?php
    ...
    if ($_POST['Submit'] == "Deactive") {
     if(!empty($_POST['suppcd']) && is_array($_POST['suppcd'])) 
     {
           ...
           foreach($_POST['suppcd'] as $value ) {
		    $sql = "Update supplier_master set Status ='D',";
            $sql .= " modified_by='$suppmoby',";
            $sql .= " modified_on='$suppmoon' WHERE SuppNo ='".$value."'";
     ...
     if(!empty($_POST['suppcd']) && is_array($_POST['suppcd'])) 
     {
           ...
           $suppmoon= date("Y-m-d H:i:s");
           foreach($_POST['suppcd'] as $value ) {
		    $sql = "Update supplier_master set Status ='A',";
            $sql .= " modified_by='$suppmoby',";
            $sql .= " modified_on='$suppmoon' WHERE SuppNo ='".$value."'";
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
        ...
        $fname = "supplier_rpt.rptdesign&__title=myReport"; 
	...
	<fieldset name="Group1" style=" width: 980px;" class="style2">
	 <legend class="title">SUPPLIER MASTER</legend>
	  <br>
		 ...
		 <tbody>
		 <?php 
		 	$sql = "SELECT SuppNo, Name, Tel, Fax, Email, Contact, Status ";
      ...
      //else { $showdte = ""; }
            ...
            echo '<td align="center"><a href="'.$urlvm.'?suppcd='.$row['SuppNo'].'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            }
			
			if ($var_accupd == 0){
            echo '<td align="center"><a href="#">[EDIT]</a>';'</td>';
            }else{
            echo '<td align="center"><a href="'.$urlpop.'?suppcd='.$row['SuppNo'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }

            echo '<td align="center"><input type="checkbox" name="suppcd[]" value="'.$row['SuppNo'].'" />'.'</td>';
