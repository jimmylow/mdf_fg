<?php
    ...
    if ($_POST['Submit'] == "Save") {
     ...
     if ($catcd <> "") {
      ...
      $var_sql = " SELECT count(*) as cnt from mdfcategory_master ";
      $var_sql .= " WHERE category_code = '$catcd'";

      $query_id = mysql_query($var_sql) or die($var_stat = mysql_error());
      $res_id = mysql_fetch_object($query_id);

      if ($res_id->cnt > 0 ) {
	     $backloc = "../main_mas/mdfcategory_mas.php?stat=3&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>"; 
      }else {
      
         $sql = "INSERT INTO mdfcategory_master values 
                ('$catcd', '$catdesc', '$var_loginid', CURDATE(), '$var_loginid', CURDATE())";

     	 mysql_query($sql); 
     	 $backloc = "../main_mas/mdfcategory_mas.php?stat=1&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";      
      } 
     }else{
       $backloc = "../main_mas/mdfcategory_mas.php?stat=4&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";
     }
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['catcd']) && is_array($_POST['catcd'])) 
     {
           ...
           foreach($_POST['catcd'] as $value ) {
		    $sql = "DELETE FROM mdfcategory_master WHERE category_code ='".$value."'"; 
		 	mysql_query($sql); 
		   }
		  $backloc = "../main_mas/mdfcategory_mas.php?stat=1&menucd=".$var_menucode;
          echo "<script>";
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
        ...
        // Redirect browser
        $fname = "mdfcategory_rpt.rptdesign&__title=myReport"; 
...
<body onload="document.InpCurrMas.catcd.focus();">
  <?php include("../topbarm.php"); ?> 
  <div class="contentc">

	<fieldset name="Group1" style=" width: 933px;" class="style2">
	 <legend class="title">OWN/MDF CATEGORY MASTER</legend>
	  <br>
	  ...
	  <form name="InpCurrMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" style="height: 134px; width: 907px;">
		<table>
	  	  <tr>
	  	    ...
	  	    <td style="width: 138px" class="tdlabel">Category Code</td>
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="catcd" id ="catid" type="text" maxlength="10" onchange ="upperCase(this.id)" size = "12" onBlur="AjaxFunction(this.value);">
			</td>
	  	  </tr>
	  	  <tr>
	  	    <td>
	  	...
	  	<?php
				$sql = "SELECT category_code, category_desc, modified_by, modified_on ";
        ...
        <form name="LstColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>">
		 ...
		 <table cellpadding="0" cellspacing="0" id="example" class="display" width="100%">
         	...
         	<tr>
          		...
          		<th class="tabheader" style="width: 138px">Category Code</th>
		 ...
		 <?php 
			...
			$urlpop = 'upd_mdfcategory_mas.php';
			echo '<tr>';
            echo '<td>'.$numi.'</td>';
            ...
            echo '<td align="center"><a href="'.$urlpop.'?catcd='.$row['category_code'].'&catde='.$row['category_desc'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }
            if ($var_accdel == 0){
              echo '<td align="center"><input type="checkbox" DISABLED  name="catcd[]" value="'.$rowq['category_code'].'" />'.'</td>';
            }else{
              echo '<td align="center"><input type="checkbox" name="catcd[]" value="'.$row['category_code'].'" />'.'</td>';
            }
            echo '</tr>';
            $numi = $numi + 1;
			}
