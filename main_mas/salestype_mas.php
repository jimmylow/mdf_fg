<?php
    ...
    if ($_POST['Submit'] == "Save") {
     $salescd   = $_POST['salescd'];
     $salesdesc = $_POST['salesde'];
     if ($salescd <> "") {

      $var_sql = " SELECT count(*) as cnt from salestype_master ";
      $var_sql .= " WHERE salestype_code = '$salescd'";

      $query_id = mysql_query($var_sql) or die($var_stat = mysql_error());
      $res_id = mysql_fetch_object($query_id);

      if ($res_id->cnt > 0 ) {
	     $backloc = "../main_mas/salestype_mas.php?stat=3&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>"; 
      }else {
      
         $sql = "INSERT INTO salestype_master values 
                ('$salescd', '$salesdesc', '$var_loginid', CURDATE(), '$var_loginid', CURDATE())";

     	 mysql_query($sql); 
     	 $backloc = "../main_mas/salestype_mas.php?stat=1&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";      
      } 
     }else{
       $backloc = "../main_mas/salestype_mas.php?stat=4&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";
     }
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['salescd']) && is_array($_POST['salescd'])) 
     {
        
           foreach($_POST['salescd'] as $value ) {
		    $sql = "DELETE FROM salestype_master WHERE salestype_code ='".$value."'"; 
		 	mysql_query($sql); 
		   }
		  $backloc = "../main_mas/salestype_mas.php?stat=1&menucd=".$var_menucode;
          echo "<script>";
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
        ...
        // Redirect browser
        $fname = "salestype_rpt.rptdesign&__title=myReport"; 
...
function stateck()
{
	...
	var url="aja_chk_salestype.php";
	
	url=url+"?salescd="+currcd;
...
<body onload="document.InpCurrMas.salescd.focus();">
  <?php include("../topbarm.php"); ?> 
  <div class="contentc">

	<fieldset name="Group1" style=" width: 933px;" class="style2">
	 <legend class="title">SALES TYPE MASTER</legend>
	  <br>
	  ...
	  <form name="InpCurrMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" style="height: 134px; width: 907px;">
		<table>
	  	  <tr>
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="salescd" id ="salesid" type="text" maxlength="5" onchange ="upperCase(this.id)" style="width: 48px" onBlur="AjaxFunction(this.value);">
			</td>
	  	  </tr>
	  	  <tr>
	  	    <td>
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="salesde" id ="currdeid" type="text" maxlength="50" onchange ="upperCase(this.id)" style="width: 354px">
			</td>
	  	  </tr>  
	  	  <tr>
	  	   <td>
	  	...
	  	<?php
				$sql = "SELECT salestype_code, salestype_desc, modified_by, modified_on ";
        ...
        <form name="LstColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>">
		 ...
		 <table cellpadding="0" cellspacing="0" id="example" class="display" width="100%">
         	...
         	<tr>
          		...
          		<th class="tabheader" style="width: 138px">Sales Type</th>
		 ...
		 <?php 
			...
			$urlpop = 'upd_salestype_mas.php';
			echo '<tr>';
            echo '<td>'.$numi.'</td>';
            echo '<td>'.$row['salestype_code'].'</td>';
            echo '<td>'.$row['salestype_desc'].'</td>';
            ...
            echo '<td align="center"><a href="'.$urlpop.'?salescd='.$row['salestype_code'].'&salesde='.$row['salestype_desc'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }
            if ($var_accdel == 0){
              echo '<td align="center"><input type="checkbox" DISABLED  name="salescd[]" value="'.$rowq['salestype_code'].'" />'.'</td>';
            }else{
              echo '<td align="center"><input type="checkbox" name="salescd[]" value="'.$row['salestype_code'].'" />'.'</td>';
            }
            echo '</tr>';
            $numi = $numi + 1;
			}
