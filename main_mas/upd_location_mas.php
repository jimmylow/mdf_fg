<?php
    ...
    if($var_loginid == "") { 
      ...
      $var_loccd  = $_GET['loccd'];
	  $var_locde = $_GET['locde'];
	  $var_menucode = $_GET['menucd'];
	}
    
     if ($_POST['Submit'] == "Update") {
    
 		    $var_loccd  = $_POST['loccdu'];
        $locdescd = $_POST['locdeu'];
        $var_menucode  = $_POST['menudcode'];
         if ($var_loccd <> "") {
        
         $vartoday = date("Y-m-d H:i:s");
         $sql = "Update location_master set location_desc ='$locdescd',";
         $sql .= " modified_by='$var_loginid',";
         $sql .= " modified_on='$vartoday' WHERE location_code = '$var_loccd'";
           
         //echo $sql;  
       	 mysql_query($sql); 
         $backloc = "../main_mas/location_mas.php?menucd=".$var_menucode;
 		     echo "<script>";
...
<body OnLoad="document.InpColMas.locdeu.focus();">
<?php include("../topbarm.php"); ?> 
   
	<div class="contentc">

	<fieldset name="Group1" style="height: 238px; width: 718px;" class="style2">
	 <legend class="title">EDIT LOCATION MASTER</legend>
	  <br>
	  ...
	  <form name="InpColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" style="height: 134px">
		...
		<table>
	  	  <tr>
	  	    ...
	  	    <?php
	  	       if (isset($var_loccd)){
	  	       echo '<input readonly="readonly" class="inputtxt" name="loccdu" id ="clrcdid" type="text" maxlength="5" onchange ="upperCase(this.id)" value="'.$var_loccd.'" style="width: 48px">';
	  	       }else{
	  	       echo '<input readonly="readonly" class="inputtxt" name="loccdu" id ="clrcdid" type="text" maxlength="5" onchange ="upperCase(this.id)" style="width: 48px">';
	  	       }
	  	    ?>
			</td>
	  	  </tr>
	  	  <tr>
	  	    ...
	  	    <td>
	  	     <?php
	  	       if (isset($var_locde)){
	  	       echo '<input class="inputtxt" name="locdeu" id ="currdeid" type="text" maxlength="50" style="width: 354px" onchange ="upperCase(this.id)" value="'.$var_locde.'">';
	  	       }else{
	  	       echo '<input class="inputtxt" name="locdeu" id ="currdeid" type="text" maxlength="50" onchange ="upperCase(this.id)" style="width: 354px" >';
	  	       }
	  	    ?>
...
<?php        
            ...
            $sql .= " where location_code = '".$var_loccd."'";
