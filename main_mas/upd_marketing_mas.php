<?php
    ...
    $var_spvcd   = htmlentities($_GET['spvcd']);
     ...
     if ($_POST['Submit'] == "Update") {
        ...
        if ($var_spvcd <> "") {
         	...
         	$sql = "Update marketing_master set mkt_name ='$spvnamecd', ";
          $sql .= " comm = '$spvcomm', salesamt = '$spvamt', ";
         	$sql .= " modified_by='$var_loginid',";
         	$sql .= " modified_on='$vartoday' WHERE mkt_code = '$var_spvcd'";
    ...
    $sql = "select comm, salesamt from marketing_master";
    $sql .= " where mkt_code = '$var_spvcd'";
    
    $tmp = mysql_query($sql) or die ("cant get master : ".mysql_error());
    
    if(mysql_numrows($tmp) > 0) {
      $data = mysql_fetch_object($tmp);
      
      $var_comm = $data->comm;
      $var_salesamt = $data->salesamt;
    }
...
<body OnLoad="document.InpColMas.spvnameu.focus();">
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style="height: 238px; width: 718px;">
	 <legend class="title">EDIT MARKETING MASTER</legend>
	  <br>
	  ...
	  <form name="InpColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" style="height: 134px">
		...
		<table>
	  	  <tr>
	  	    ...
	  	    <?php
	  	       if (isset($var_spvcd)){
	  	       echo '<input readonly="readonly" class="textnoentry" name="spvcdu" id ="spvcdid" type="text" maxlength="15" size="17" onchange ="upperCase(this.id)" value="'.$var_spvcd.'">';
	  	       }else{
	  	       echo '<input readonly="readonly" class="textnoentry" name="spvcdu" id ="spvcdid" type="text" maxlength="15" size="17" onchange ="upperCase(this.id)">';
	  	       }
	  	  ...
	  	  <tr>
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="comm" id ="commid" type="text" maxlength="10" style="width: 100px; text-align : right;" value="<?php echo $var_comm; ?>">
			</td>
	  	  </tr>  
	   	   <tr>
	   	    <td>
	  	    ...
	  	    <td>:</td>
	  	    <td>
			<input class="inputtxt" name="salesamt" id ="salesamtid" type="text" maxlength="10" style="width: 100px; text-align : right;" value="<?php echo $var_salesamt; ?>">
			</td>
	  	  </tr>        
<?php        
