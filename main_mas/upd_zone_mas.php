<?php
    ...
    if($var_loginid == "") { 
      ...
      $var_zonecd  = $_GET['zonecd'];
	    $var_zonede = $_GET['zonede'];
	    $var_menucode = $_GET['menucd'];
	}
    
     if ($_POST['Submit'] == "Update") {
    
 		    $var_zonecd  = $_POST['zonecdu'];
        $zonedescd = $_POST['zonedeu'];
        $var_menucode  = $_POST['menudcode'];
         if ($var_zonecd <> "") {
        
         $vartoday = date("Y-m-d H:i:s");
         $sql = "Update zone_master set zone_desc ='$zonedescd',";
         $sql .= " modified_by='$var_loginid',";
         $sql .= " modified_on='$vartoday' WHERE zone_code = '$var_zonecd'";
...
<body OnLoad="document.InpColMas.zonedeu.focus();">
<?php include("../topbarm.php"); ?> 
   
	<div class="contentc">

	<fieldset name="Group1" style="height: 238px; width: 718px;" class="style2">
	 <legend class="title">EDIT ZONE MASTER</legend>
	  <br>
	  ...
	  <form name="InpColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" style="height: 134px">
		...
		<table>
	  	  <tr>
	  	    ...
	  	    <?php
	  	       if (isset($var_zonecd)){
	  	       echo '<input readonly="readonly" class="inputtxt" name="zonecdu" id ="clrcdid" type="text" maxlength="5" onchange ="upperCase(this.id)" value="'.$var_zonecd.'" style="width: 48px">';
	  	       }else{
	  	       echo '<input readonly="readonly" class="inputtxt" name="zonecdu" id ="clrcdid" type="text" maxlength="5" onchange ="upperCase(this.id)" style="width: 48px">';
	  	       }
	  	    ?>
			</td>
	  	  </tr>
	  	  <tr>
	  	    ...
	  	    <td>
	  	     <?php
	  	       if (isset($var_zonede)){
	  	       echo '<input class="inputtxt" name="zonedeu" id ="zonedeid" type="text" maxlength="50" style="width: 354px" onchange ="upperCase(this.id)" value="'.$var_zonede.'">';
	  	       }else{
	  	       echo '<input class="inputtxt" name="zonedeu" id ="zonedeid" type="text" maxlength="50" onchange ="upperCase(this.id)" style="width: 354px" >';
	  	       }
             
            $sql = " select create_by, create_on, modified_by, modified_on";
            $sql .= " from zone_master ";
            $sql .= " where zone_code = '".$var_zonecd."'";
