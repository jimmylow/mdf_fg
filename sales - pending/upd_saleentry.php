 ...
 //--------------add here by cedric ----------------------//
    ...
    //phpinfo();
        ...
        $vartoday = date("Y-m-d H:i:s"); 
				...
				//mysql_query($sql) or die("Error update Sale Entry  :".mysql_error(). ' Failed SQL is --> '. $sql);       	
				
				$sql = "DELETE from salesentrydet WHERE sordno = ''"; 
				...
				{	
					foreach($_POST['prococode'] as $row=>$matcd ) {
						...
						if ($matcode <> "" && $matqty <> "")
						{
							...
							//mysql_query($sql) or die("Error update Sales Entry Details :".mysql_error(). ' Failed SQL is --> '. $sql);       	
           				}	
					}
				}			
			...
			$backloc = "../sales/sale_mas.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}  
     ...
     <table id="itemsTable" class="general-table">
            ...
            <tbody>
             ...
             <tr class="item-row">
                ...
                <td>
				<input name="prococode[]" tProItem1=1 id="prococode<?php echo $i; ?>" tabindex="0" class="autosearch" style="width: 161px" onchange ="upperCase(this.id)" value ="<?php echo htmlentities($rowq['sprocd']); ?>" onblur="getUprice('<?php echo $i; ?>')"></td>
				<td>
				<input name="procouom[]" id="procouom<?php echo $i; ?>" class="tInput" readonly="readonly" style="border-style: none; border-color: inherit; border-width: 0; width: 75px" value ="<?php echo $rowq['sprouom']; ?>"></td>
				<td >
				<input name="procouqty" value="<?php echo $var_uqty; ?>" class="tInput" id="procouqty" style=" border-style: none; border-color: inherit; border-width: 0; width: 48px; text-align : right" readonly="readonly"></td>
                <td>
				<input name="procotype[]" class="tInput" id="procotype" readonly="readonly" style="border-style: none; border-color: inherit; border-width: 0; width: 75px;" value ="<?php echo $rowq['sptype']; ?>"></td>
                <td>
				<input name="procoprice[]" class="tInput" id="procoprice<?php echo $i; ?>" style=" border-style: none; border-color: inherit; border-width: 0; width: 75px; text-align : right" value ="<?php echo $rowq['sprounipri']; ?>" onBlur="calcCost('<?php echo $i; ?>');"></td>
				<td >
				<input name="procoqty" id="procoqty" style="width: 48px; text-align : right" value ="<?php echo $rowq['sproqty']; ?>" onBlur="get_totpcs()"></td>
				<td >
				<input name="procototpcs" class="tInput" id="procototpcs" style=" border-style: none; border-color: inherit; border-width: 0; width: 48px; text-align : right" readonly="readonly" value ="<?php echo $var_totpcs; ?>"></td>              
