<?php
    ...
    $vmordno = $_POST['sordno'];
        ...
        $vartoday = date("Y-m-d H:i:s"); 
				...
				//$sql = "Update salesshipmas Set scustcd = '$vmcustcd', shipdte ='$vmshipdte', ";
        $sql = "Update salesshipmas Set shipdte ='$vmshipdte', ";
				$sql .= "                    stype = '$vmtype', sprinted = '$vmprinted', ";
			...
			$backloc = "../sales/upd_shipping.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		} 
...
function validateForm()
{
  ...
  var x=document.forms["InpPO"]["stype"].value;
    ...
    for (var j = 1; j < rowCount; j++){
       ...
       var idrowtotship = "procototpcs"+j;
       var rowItem = document.getElementById(idrowItem).value;	 
       var rowship = document.getElementById(idrowship).value;	 
       var rowtotship = document.getElementById(idrowtotship).value;	
        ...
        if (req)
	  	{
			req.onreadystatechange = function()
			{
				if (req.readyState == 4)
				{
					...
					if (req.status == 200)
					{
            //alert (rowItem+' 1 - ' +parseInt(req.responseText)+ ' 2 - ' + parseInt(rowship));
						if (parseInt(req.responseText) < parseInt(rowtotship))
						{
						   flgchk = 0;
						   alert ('Insufficient Balance : '+ rowItem + ' At Row '+j+" Sys Qty : "+req.responseText+" Ship : "+rowtotship);
						   return false;
						}
					} else {
						//alert("There was a problem while using XMLHTTP:\n" + req.statusText+req.status);
	  ...
	  <form name="InpPO" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 993px; ">
		  <tr>
		   ...
		   &nbsp;</td>
	  	   ...
	  	   <td style="width: 13px">:</td>
	  	   <td style="width: 201px">
		   	<select name="sacustcd" id="sacustcd" style="width: 268px" readonly class="textnoentry">
			 <?php
       ...
       <td style="width: 10px"></td>
        ...
        <input name="procouqty[]" value="<?php echo $var_uqty; ?>" id="procouqty<?php echo $i; ?>" type="hidden"></td>                
                ...
                <td>
				<input name="procoshipqty[]" id="procoshipqty<?php echo $i; ?>" style="width: 48px; text-align : right" value ="<?php echo $rowq['shipqty']; ?>" onBlur="get_totpcs(<?php echo $i; ?>)"></td>
                <td>
