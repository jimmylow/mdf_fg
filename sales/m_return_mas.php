<?php
    ...
    if($var_loginid == "") { 
	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['srtnno']) && is_array($_POST['srtnno'])) 
     	{
           foreach($_POST['srtnno'] as $key) {
             $defarr = explode(",", $key);
             //print_r($defarr);
             $var_sale = $defarr[0];
             $var_cust = $defarr[2];
			 ...
			 $sql  = "Update salesreturn Set stat = 'C', modified_by = '$var_loginid', modified_on = '$vartoday' ";
             $sql .=	" Where srtnno ='".$var_sale."' And scustcd='".$var_cust."'";
             //echo $sql;
		   ...
		   $backloc = "../sales/m_return_mas.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
   ...
   if ($_POST['Submit'] == "Active") {
     	if(!empty($_POST['srtnno']) && is_array($_POST['srtnno'])) 
     	{
           foreach($_POST['srtnno'] as $key) {
		     ...
		     $vartoday = date("Y-m-d H:i:s");
			 $sql  = "Update salesreturn Set stat = 'A', modified_by = '$var_loginid', modified_on = '$vartoday' ";
             $sql .=	" Where srtnno ='".$var_sale."' And scustcd='".$var_cust."'";
...
<body>
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 900px;" class="style2">
	 <legend class="title">CUSTOMER SALES RETURN LISTING</legend>
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                ...
                if ($var_accadd == 0){
    	  	   ...
    	  	   $msgdel = "Are You Sure Delete Selected Sales Return?";
          ...
          /*
			   ...
			   $msgdel = "Are You Sure Active Selected Sales Return?";
    	  	   if ($var_accdel == 0){
   					echo '<input disabled="disabled" type=button name = "Submit" value="Cancel" class="butsub" style="width: 60px; height: 32px">';
  				}else{
		 ...
		 <tbody>
		 <?php 
		    $sql = "SELECT srtnno, srtndte, scustcd, srefno, stat, posted ";
		    $sql .= " FROM salesreturn";
    		$sql .= " ORDER BY srtnno desc";  
			$rs_result = mysql_query($sql); 
	
		    $numi = 1;
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
			
				$srtnno = htmlentities($rowq['srtnno']);
				$rtndte = date('d-m-Y', strtotime($rowq['srtndte']));
			...
			/*	$sql1 = "select app_stat from salesappr";
				...
				$urlpop = 'upd_salereturn.php';
				$urlvie = 'vm_salereturn.php';
				//$urlvie = 'ship_mas.php';
            	...
            	echo '<td>'.$rowq['srefno'].'</td>';
            	echo '<td>'.$rowq['stat'].'</td>';
            
            	if ($var_accvie == 0){
            		echo '<td align="center"><a href="#">[VIEW]</a>';'</td>';
            	}else{
            		echo '<td align="center"><a href="'.$urlvie.'?sorno='.$srtnno.'&custcd='.$rowq['scustcd'].'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            	}
	            if ($var_accupd == 0){
		            echo '<td align="center"><a href="#">[EDIT]</a>';'</td>';
	            }else{
	            	if ($rowq['posted'] == "Y" || $rowq['stat'] == "C"){
	            		echo '<td align="center"><a href="#" title="This Sales Return Is Posted / Cancelled; Edit Is Not Allow">[EDIT]</a>';'</td>';
	            	}else{ 
		            	echo '<td align="center"><a href="'.$urlpop.'?sorno='.$srtnno.'&custcd='.$rowq['scustcd'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
	            	}
	            }
	            if ($var_accdel == 0){
				  ...
				  //}else{	
	              	$values = implode(',', $rowq);	
	              	echo '<td align="center"><input type="checkbox" name="srtnno[]" value="'.$values.'" />'.'</td>';
    	        //  }	
    	        }
