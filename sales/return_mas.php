<?php
    ...
    $vmremark = $_POST['saremark'];
		...
		if ($vmcustcd <> "") {
            ...
            /*----------------------------- Cash Bill details ------------------------------------ */
              $chk_invno_query = mysql_query("select count(*) from `ctrl_sysno` where `descrip` = 'SALESRTN' and counter = 'HQ'; ", $db_link);

              $chk_invno_res = mysql_fetch_array($chk_invno_query) or die("cant Get Sales Return Info".mysql_error());
              
              if ($chk_invno_res[0] > 0 ) {
                  $get_invno_query = mysql_query("select noctrl from `ctrl_sysno` where `descrip` = 'SALESRTN' and counter = 'HQ' ", $db_link);
                  
                  $get_invno_res = mysql_fetch_object($get_invno_query) or die("Cant Get Sales Return 2 ".mysql_error()); 
 		  ...
 		  mysql_query("update `ctrl_sysno` set `noctrl` = `noctrl` + 1
                           where `descrip` = 'SALESRTN' and counter = 'HQ'", $db_link) 
                           or die("Cant Update Sales Return Auto No ".mysql_error());              
               
                }  else { 

		   mysql_query("insert into `ctrl_sysno` 
                          (`descrip`, `counter`, `noctrl`)
                   values ('SALESRTN', 'HQ', 1);",$db_link) or die("Cant Insert Into Sales Return Auto No ".mysql_error());
        ...
        $vartoday = date("Y-m-d H:i:s"); 
				...
				$sql = "INSERT INTO salesreturn values 
						('$var_invno', '$vmrtndte','$vmcustcd','$vmtype','$vmrefno','$var_loginid','$vartoday', 
						 '$var_loginid', '$vartoday', 'A', 'N', '$vmremark')";
            ...
            $matrejuom  = $_POST['prorejuom'][$row];
              ...
              if ($matothqty == "" or empty($matothqty)){$matothqty = 0;}
							...
							$sql = "INSERT INTO salesreturndet values 
						    		('$var_invno', '$matcode', '$matqty','$matuom', '$matuprice', '$matrejqty', '$matrejcode', '$matrejuom', '$matothqty', '$matseqno')";
			...
			$backloc = "../sales/return_mas.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}  
...
function calcAmt(vid)
{
    ...
    document.getElementById(vprotqty).value = parseFloat(totqty).toFixed(0);
 ...
 <!-- <?php include("../sidebarm.php"); ?> -->
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 973px;" class="style2">
	 <legend class="title">CUSTOMER SALES RETURN ENTRY</legend>
	  <br>	 
	  ...
	  <form name="InpPO" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 993px; ">
		   ...
		   <input class="inputtxt" name="sartndte" id ="sartndte" type="text" style="width: 128px;" value="<?php  echo date("d-m-Y"); ?>">
		  ...
		  <tr>
	  	   ...
	  	   <td style="width: 13px">:</td>
	  	   <td style="width: 201px">
			<input class="inputtxt" name="srefno" id="srefnoid" type="text" maxlength="45" style="width: 204px;" onchange ="upperCase(this.id)"></td>		   </td>
		   <td style="width: 10px"></td>
		   ...
		   <td style="width: 284px">
			 ...
			 <?php
			  ...
			  if(mysql_num_rows($sql_result)) 
			  {
			   while($row = mysql_fetch_assoc($sql_result)) 
			   { 
				echo '<option value="'.$row['shiptype_code'].'">'.$row['shiptype_code']." | ".$row['shiptype_desc'].'</option>';
			   } 
		      } 
	         ?>				   
	       </select>
		  ...
		  <table id="itemsTable" class="general-table" style="width: 958px">
            ...
            <tbody>
             <tr class="item-row">
                ...
                <td>
				<input name="procd[]" value="" tProCd1=1 id="procd1" class="autosearch" style="width: 100px" onchange ="upperCase(this.id)" ></td>
                <td>
				<input name="procdname[]" value="" id="proconame1" readonly="readonly" style="border-style: none; border-color: inherit; border-width: 0; width: 150px;"></td>
                <td>
				<input name="prouom[]" id="prouom1" readonly="readonly" style="width: 50px; border-style: none; border-color: inherit; border-width: 0;">
				</td>
                <td >
				<input name="prooupri[]" id="prooupri1" style="border-style: none; width: 70px; text-align:right;" readonly="readonly">
				</td>
                <td >
				<input name="proorqty[]" id="proordqty1"  style="width: 50px; text-align:center;">
				</td>        
                <td >
				<input name="prorejqty[]" id="prorejqty1"  style="width: 50px; text-align:center;">
				</td>        
                <td >
				<input name="rejprocd[]" value="" id="rejprocd1" class="autosearch" style="width: 100px" onchange ="upperCase(this.id)" onBlur="chkrejqty('1');"></td>
                <td>
				<input name="prorejuom[]" id="prorejuom1" readonly="readonly" style="width: 50px; border-style: none; border-color: inherit; border-width: 0;" >
				</td>

                <td >
				<input name="proothqty[]" id="proothqty1" onBlur="calcAmt('1');" style="width: 50px; text-align:center;">
				</td> 
         <td >
				<input name="protqty[]" id="protqty1" readonly style="width: 50px; border-style: none; border-color: inherit; border-width: 0; text-align:center;" >
				</td>                  
				<td >
				<input name="proouamt[]" id="proouamt1" readonly="readonly" style="width: 80px; border-style: none; border-color: inherit; border-width: 0; text-align:right;">
				</td>
             </tr>
            </tbody>           
           </table>
		  <table class="general-table" style="width: 958px">
          	 <tr>
              <td style="width: 870px; text-align:right" >Total : </td>              
              <td align="right">
              <input readonly="readonly" name="totamt" id ="totamtid" type="text" style="width: 80px;" class="textnoentry1">
              </td>
             </tr>
        </table>               
